...
public sealed class RevolutionaryRuleSystem : GameRuleSystem
{
    ...
    private void OnMobStateChanged(MobStateChangedEvent ev)
    {
        ...
        if (_aliveRevoHeads.TryFirstOrNull(x => x.Key is not null && x.Key.OwnedEntity == ev.Target, out var revohead))
        {
            ...
            _aliveRevoHeads[revohead.Value.Key] = ev.NewMobState != MobState.Dead; // Literally used by ICharacherDeadIC which isn't deprecated, dont @ me
        ...
        {
            _aliveCommandHeads[staffhead.Value.Key] = ev.NewMobState != MobState.Dead;
